  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
        ? "bg-[#0d1420]/95 backdrop-blur-md py-2 shadow-2xl border-b border-white/5"
    ...
    >
      <div className="container mx-auto px-6 lg:px-12">
        <nav className="flex items-center justify-between">
          ...
          <div className={`hidden lg:flex items-center gap-1 transition-all duration-300 rounded-full px-2 py-1.5 ${isScrolled ? "bg-white/5" : "bg-white/50 backdrop-blur-md shadow-lg"
            }`}>
            {navItems.map((item) => {

    ...
    <section ref={sectionRef} className="py-24 lg:py-32 bg-zinc-950 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="max-w-3xl mb-16 lg:mb-24">
          ...
          <h2
            className={`text-4xl md:text-5xl lg:text-7xl font-bold text-white mb-6 leading-tight transition-all duration-700 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
          >
            One Integrated Group
          </h2>
          <p
            className={`text-xl text-zinc-400 transition-all duration-700 delay-200 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
          >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {networkLocations.map((loc, idx) => (
            <div
              key={idx}
              className={`group bg-zinc-900/50 border border-zinc-800 hover:border-sky-500/50 rounded-3xl p-8 hover:bg-zinc-900 transition-all duration-500 cursor-pointer relative overflow-hidden ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
                }`}
          ...
          <div
            className={`bg-zinc-800/20 border border-zinc-800/50 rounded-3xl p-8 flex items-center justify-center relative overflow-hidden ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}

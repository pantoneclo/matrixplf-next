...
export default function CategorySection() {
  ...
  const columns = useMemo(() => {
    ...
    while (i < categories.length) {
      if (isSingle || i === categories.length - 1) {
        ...
        cols.push({
          type: 'single',
          width: 'w-[70vw] sm:w-[300px] md:w-[300px] lg:w-[360px]',
          items: [categories[i]]
        })
        i += 1
      } else {
  ...
  // Desktop only: horizontal movement driven by vertical scroll progress
  const xMovement = useTransform(scrollYProgress, [0, 1], [0, -scrollRange])
  const smoothX = useSpring(xMovement, { stiffness: 60, damping: 25 })

  // Shared card columns renderer
  const renderColumns = (cols: typeof columns) =>
    cols.map((col, idx) => (
      <div key={idx} className={`flex-shrink-0 flex flex-col gap-2 md:gap-4 ${col.width}`}>
        {col.type === 'single' ? (
          <CategoryCard
  ...
  return (
    <div className="relative bg-[#0d1420]">
      ...
      <div className="lg:hidden pb-8">
        <div
          className="flex gap-2 items-stretch h-[75vh] pl-6 pr-6 overflow-x-auto"
          style={{ scrollbarWidth: 'none', WebkitOverflowScrolling: 'touch' } as React.CSSProperties}
        >
          {renderColumns(columns)}
          {CTACard}
        </div>
        <p className="text-center text-slate-600 text-[10px] uppercase tracking-widest mt-4 font-bold">
          ← Swipe to explore →
        </p>
      </div>

      {/* ─── DESKTOP: Scroll-driven sticky horizontal animation (unchanged) ─── */}
        ...
        <section className="sticky top-0 h-screen w-full flex flex-col justify-center overflow-hidden">
          <div className="w-full relative z-10 flex items-center">
            ...
            <motion.div
              ref={trackRef}
              style={{ x: smoothX }}
              className="flex gap-4 items-stretch h-[85vh] pl-12 pr-12"
            >
              {renderColumns(columns)}
              {CTACard}
            </motion.div>
